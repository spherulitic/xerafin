...
//       ctx.fillText(this.invadersAlphas[i].alphagram, this.invadersAlphas[i].x, this.invadersAlphas[i].y);
       ctx.fillText(alphaSortMethod(this.invadersAlphas[i].alphagram, Number(localStorage.gAlphaSortInput)), this.invadersAlphas[i].x, this.invadersAlphas[i].y);
    }
...
//-------------------------------------------------------------------------------------------------------------------------------										   
	submitAnswer:function() {
		...
		if (this.invaderStatus == 'finished' || this.invaderStatus == 'paused') {
			...
			if (found) {$('#invAnswerBox').toggleClass('slothFlashCorrect',700).delay(300).toggleClass('slothFlashCorrect',700);}
			else {$('#invAnswerBox').toggleClass('flashTypo',700).delay(300).toggleClass('flashTypo',700);}
		}	
	},
//-------------------------------------------------------------------------------------------------------------------------------										   
	getAlpha:function() {
...
//-------------------------------------------------------------------------------------------------------------------------------										   
	markAsIncorrect:function(alphaObj) {
		var  d = {user: userid, question: alphaObj.alphagram, correct: false, cardbox: alphaObj.cardbox, incrementQ: true};
		slothSubmitQuestion(d);
...
//-------------------------------------------------------------------------------------------------------------------------------
	generateDOM:function() {
		gCreateElemArray([ 
			['a','div','quizContent invWrapper','invadersWrapper','content_pan_1_c',''],
...
function Invader(){
	...
	this.colorList = ["black", "azure", "mediumorchid", "palegreen", "yellowgreen", "khaki", "salmon", "hotpink", "crimson", "magenta", "orange", "peachpuff", "mistyrose", "floralwhite",'white'];
	this.currentScore = 0;
...
function initInvaders() {
	...
	if (!document.getElementById('invadersCanvas')){
		...
		laserSound2=createNewAudio("laserSound",{src:'audio/laserShot2.mp3'},"content_pan_1_c");
